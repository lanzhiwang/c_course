...
返回整数 n 的二进制原码表示
 */
char *int_to_binary(int n, char *binary)
{
...
返回整数 n 的反码表示
 */
char *int_to_one_complement(int n, char *one_complement)
{
	// 获取原码二进制表示
...
函数返回整数 n 的补码表示
 */
char *int_to_two_complement(int n, char *two_complement)
{
	// 获取反码
	char *one_complement =
	    (char *)malloc(sizeof(char) * (sizeof(int) * 8 + 1));
	int_to_one_complement(n, one_complement);
...
int main(void)
{
	...
	for (i = -6; i < 7; i++) {
		...
		char *one_complement =
		    (char *)malloc(sizeof(char) * (sizeof(int) * 8 + 1));
		int_to_one_complement(i, one_complement);

		// 获取补码
		char *two_complement =
		    (char *)malloc(sizeof(char) * (sizeof(int) * 8 + 1));
		int_to_two_complement(i, two_complement);
